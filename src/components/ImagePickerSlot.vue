...
<template>
  <div class="flex-1 min-w-0 flex flex-col gap-1.5" :class="{ 'opacity-45 pointer-events-none': disabled }">
    <div class="text-[11px] font-semibold uppercase text-text-dim tracking-[0.04em]">{{ label }}</div>
    <div
      class="group/slot relative w-full aspect-square rounded-[6px] overflow-hidden border cursor-pointer outline-none"
      :class="error
    ...
    >
      ...
      <div v-else class="w-full h-full flex items-center justify-center p-2">
        <span class="text-[11px] text-text-dim text-center leading-snug">{{ emptyText ?? 'Select Icon' }}</span>
      </div>
      <button
        v-if="path"
        class="absolute top-1 right-1 w-[18px] h-[18px] rounded-full bg-black/60 border-none text-white text-[13px] leading-none cursor-pointer flex items-center justify-center p-0 z-[1] transition-colors duration-100 hover:bg-danger"
      ...
      <div v-if="!disabled" class="absolute inset-0 flex items-center justify-center bg-black/45 opacity-0 transition-opacity duration-150 pointer-events-none group-hover/slot:opacity-100 group-focus-visible/slot:opacity-100">
        <span class="text-[11px] font-semibold text-white uppercase tracking-[0.05em]">Change Icon</span>
      </div>
    </div>
    <div v-if="error" class="text-[11px] text-danger leading-snug break-all">File not found: {{ error }}</div>
  </div>
</template>
